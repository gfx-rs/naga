...
float4 foo_vert(uint vi : SV_VertexID) : SV_Position
{
    ...
    float baz = foo;
    foo = 1.0;
    float3x4 matrix_ = float3x4(asfloat(bar.Load4(0+0)), asfloat(bar.Load4(0+16)), asfloat(bar.Load4(0+32)));
...
float4 foo_frag() : SV_Target0
{
    ...
    {
        float3x4 _value2 = float3x4(float3(0.0.xxx), float3(1.0.xxx), float3(2.0.xxx), float3(3.0.xxx));
