...
void main() {
    ...
    color_1 = vec3(0.05, 0.05, 0.05);
    {
        ...
        while(true) {
            ...
            loop_init = false;
            uint _e8 = i_1;
            uint _e12 = _group_0_binding_0_fs.num_lights.x;
            if ((_e8 < min(_e12, 10u))) {
            } else {
                break;
            }
            {
                uint _e18 = i_1;
                Light light = _group_0_binding_1_fs[_e18];
                uint _e23 = i_1;
                float _e21 = fetch_shadow(_e23, (light.proj * in_1.world_position));
                vec3 light_dir = normalize((light.pos.xyz - in_1.world_position.xyz));
                float diffuse = max(0.0, dot(normal_1, light_dir));
                vec3 _e43 = color_1;
                color_1 = (_e43 + ((_e21 * diffuse) * light.color.xyz));
            }
        }
    }
    vec3 _e50 = color_1;
    vec4 _e55 = _group_1_binding_0_fs.color;
    _fs2p_location0 = (vec4(_e50, 1.0) * _e55);
    return;
}
